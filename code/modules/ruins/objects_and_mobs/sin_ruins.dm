...
/obj/effect/gluttony/CanPass(go/mover, turf/target, height=0)//So bullets will fly over and stuff.
	if(height==0)
		return 1
	if(ishuman(mover))
...
/obj/item/weapon/kitchen/knife/envy/afterattack(go/AM, mob/living/carbon/human/user, proximity)
	..()
	if(!proximity)
		return
	if(!istype(user))

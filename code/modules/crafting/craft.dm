...
/datum/personal_crafting/proc/del_reqs(datum/crafting_recipe/R, mob/user)
	...
	main_loop:
		for(var/A in R.reqs)
			if(ispath(A,/obj/item/blueprint))
				continue
			amt = R.reqs[A]
...
/datum/personal_crafting/ui_data(mob/user)
	...
	for(var/rec in crafting_recipes)
		...
		if(R.category != cur_category || !R.default)
			continue
	...
	for(var/obj/item/blueprint/B in environment)
		var/datum/crafting_recipe/R = get_craft_recipe(B.recipe)
...
proc/get_craft_recipe(craft_type)
	for(var/rec in crafting_recipes)
		if(istype(rec,craft_type))
			return rec
	return null
